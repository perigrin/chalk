# ABOUTME: Binary float multiplication node in the IR graph
# ABOUTME: Represents multiplication of two float operands (left * right)
use 5.42.0;
...
class Chalk::IR::Node::MulF {
    ...
    # Algebraic simplification for float multiplication
    method idealize() {
        my $left_type = $left->compute();
        my $right_type = $right->compute();
        ...
        # 1.0 * x -> x (identity left)
        if ($left_type->is_constant && $left_type->value == 1.0) {
            return $right;
        }

        # x * 0.0 -> 0.0 (zero absorption right)
        ...
        # 0.0 * x -> 0.0 (zero absorption left)
        # Only fold if both operands are constant to preserve side effects
