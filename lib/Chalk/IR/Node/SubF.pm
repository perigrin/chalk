# ABOUTME: Binary float subtraction node in the IR graph
# ABOUTME: Represents subtraction of two float operands (left - right)
use 5.42.0;
use experimental qw(class);
use utf8;

class Chalk::IR::Node::SubF {
    ...
    # Algebraic simplification for float subtraction
    method idealize() {
        my $left_type = $left->compute();
        my $right_type = $right->compute();
        ...
        # Note: No left identity (0.0 - x ≠ x)
        # Note: No swap canonicalization (subtraction is not commutative)
