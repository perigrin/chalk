...
"build_return_node: value_node is not an IR node object (got $ref_type)";
    ...
    method build_add_node( $left_node, $right_node, $source_info = undef ) {
        ...
        if ( defined($source_info) ) {
            ...
            if ( defined($left_type) || defined($right_type) ) {
                $validator->validate_type_operation( 'Add', $left_type,
    ...
    method build_multiply_node( $left_node, $right_node, $source_info = undef )
    {
        ...
        if ( defined($source_info) ) {
            ...
            if ( defined($left_type) || defined($right_type) ) {
                $validator->validate_type_operation( 'Multiply', $left_type,
                    $right_type, $source_info );
            }
        }
    ...
    method build_sub_node( $left_node, $right_node, $source_info = undef ) {
        ...
        if ( defined($source_info) ) {
            ...
            if ( defined($left_type) || defined($right_type) ) {
                $validator->validate_type_operation( 'Subtract', $left_type,
                    $right_type, $source_info );
            }
        }
    ...
    method build_divide_node( $left_node, $right_node, $source_info = undef ) {
        ...
        if ( defined($source_info) ) {
            ...
            if ( defined($left_type) || defined($right_type) ) {
                $validator->validate_type_operation( 'Divide', $left_type,
                    $right_type, $source_info );
            }
        }
   ...
   # This ensures subsequent validation operations see the newly stored variable
    ...
    method build_region_node( $source_info = undef, @control_inputs ) {
        ...
        if ( defined($source_info) ) {
            $validator->validate_control_merge( \@control_inputs,
                $source_info );
        }
    ...
    method build_loop_node( $entry_control = undef ) {
        my $ctrl = $entry_control // $current_control
    ...
    method build_loop_phi_node( $loop_node, $initial_value,
        $loop_value = undef )
    {
    ...
    method build_call_node( $function_name, $source_info = undef, @arg_nodes ) {
        ...
        if ( defined($source_info) ) {
            ...
            $validator->validate_call_arity( $function_name, $arg_count,
                $source_info );
        }
        ...
        my $call       = Chalk::IR::Node->new(
            ...
            op     => 'Call',
            inputs =>
              [ $current_control, $current_control, map { $_->id } @arg_nodes ],
    ...
    method end_loop_tracking() {
        ...
        if ( $loop_depth > 0 ) {
            ...
            pop $loop_modified_vars->@*
              if scalar( $loop_modified_vars->@* ) > 0;
        }
        return;
    }
        ...
        $source_info = undef )
    {
        ...
        if ( defined($source_info) ) {
            ...
            if ( defined($class_name) ) {
                $validator->validate_class_field( $class_name, $field_name,
                    $source_info );
            }
        }
...
# Create ArraySet node for setting array element with context extension (immutable)
 ...
 # Create HashSet node for setting hash value with context extension (immutable)
    ...
    method build_str_substr_node( $string_node, $offset_node, $length_node ) {
        ...
        my $str_substr = Chalk::IR::Node->new(
            ...
            op     => 'StrSubstr',
            inputs => [
                $current_control, $string_node->id,
    ...
    method build_scalar_ref_node( $var_name, $source_info = undef ) {
        ...
        if ( defined($source_info) ) {
            $target_node_or_id =
              $validator->validate_reference_target( $label, $source_info );
        }
        else {
        ...
        my $reference = Chalk::IR::Node::Reference->new(
            id     => $node_id,
            inputs => [ $current_control, $target_node_id ]
    ...
    method build_element_ref_node( $collection_name, $index_node ) {
        ...
        my $reference = Chalk::IR::Node::Reference->new(
            id     => $node_id,
            inputs =>
              [ $current_control, $collection_node->id, $index_node->id ],
 ...
 # This updates the variable that the reference points to
        $context = Chalk::IR::Context->extend_context( $context, $target_label,
            $value_node );
