...
class Chalk::Bootstrap::Earley {
    ...
    method _run_parse($input) {
        ...
        # Global terminal pre-scan: for each unique terminal pattern in the
        # grammar, find all positions where it matches via \G-anchored regex.
        # Populates the entire scan cache upfront so every _scan call during
        # parsing becomes a hash lookup (zero regex calls during the parse).
        # Cost: O(patterns * input_length) regex calls, typically <1% of
        # total parse time for large files.
        {
            my %seen_patterns;
            for my $cid (0 .. $core_index->count() - 1) {
                ...
                my $re = $regex_cache{$pstr} //= qr/$pstr/;
                for (my $p = 0; $p < $n; $p++) {
                    pos($input) = $p;
                    if ($input =~ /\G($re)/) {
                        $_scan_cache{$p}{$pstr} = $p + length($1);
                    } else {
                        $_scan_cache{$p}{$pstr} = undef;
                    }
                }
            }
        }
        ...
        for my $pos (0 .. $n) {
            ...
            # Terminal clustering is handled by the global pre-scan above.
            # All pattern/position combinations are already in %_scan_cache,
            # so _scan calls get immediate hash hits with zero regex overhead.
