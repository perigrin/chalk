...
class Chalk::Bootstrap::Perl::Target::XS :isa(Chalk::Bootstrap::Target) {
    ...
    method _is_stale_merge($xs_output) {
        return ($xs_output =~ /call_method\(/ && $xs_output =~ /(?:RETVAL|retval) = newSVpvs\("/);
    }
        ...
        if (defined $class_decl) {
            ...
            for my $item (@method_items) {
                ...
                if (ref($result) eq 'HASH') {
                    ...
                    # Check helper output for stale merge / eval fallback markers
                    # (the xsub wrapper is a thin delegator that won't contain them)
    ...
    # The actual return type from _emit_xs_complex_method is patched in later.
    method _scan_class_methods($class_decl) {
        ...
        for my $item ($body->@*) {
            ...
            # Updated after emission with actual return type from the helper.
            $methods{$name} = {
                returns => true,
                params  => \@param_names,
            };
    ...
    method _emit_xs_method_call_expr($node, $declared_vars) {
        ...
        if ($invocant_expr eq 'self' && defined $_class_methods
            ...
            if (@pre_eval) {
                # Pre-evaluated args still needed for complex sub-expressions
