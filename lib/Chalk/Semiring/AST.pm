...
class Chalk::Semiring::ASTElement :isa(Chalk::Element) {
    ...
    field $children :param :reader = [];      # Array of child ASTElements or terminal values
    ...
    method add($other, $swap = undef) {
        ...
        return $self unless ref($other) && $other->isa('Chalk::Semiring::ASTElement');

        # If self has no rule (is identity), return other
        return $other if !defined($rule_name) && !defined($terminal) && scalar($children->@*) == 0;

        # If other has no content, return self
        return $self if !defined($other->rule_name) && !defined($other->terminal) && scalar($other->children->@*) == 0;
    ...
    method multiply($other, $swap = undef) {
        ...
        if ($ENV{DEBUG_AST_MUL}) {
            ...
            my $other_rule = $other->rule_name // ($other->terminal ? "term:" . $other->terminal : 'undef');
...
class Chalk::Semiring::AST :isa(Chalk::Semiring) {
    ...
    ADJUST {
        # Identity elements
        $add_id = Chalk::Semiring::ASTElement->new(
            rule_name => undef,
            children => [],
            start_pos => 0,
            end_pos => 0
        ...
        $mul_id = Chalk::Semiring::ASTElement->new(
            rule_name => undef,
            children => [],
            start_pos => 0,
            end_pos => 0
    ...
    method on_complete($completed_item, $completed_element, $metadata_element = undef) {
        ...
        if ($ENV{DEBUG_AST}) {
            warn "AST on_complete: $rule_name, element rule=" . ($completed_element->rule_name // 'undef') . "\n";
        }
        ...
        for my $child ($completed_element->children->@*) {
            ...
            if (ref($child) && $child->can('terminal') && defined($child->terminal)) {
                my $term = $child->terminal;
                next if $term =~ m/^\s*$/;  # Skip whitespace-only
            }
            # Skip WS_OPT and WS_ELEMENT rule nodes
            if (ref($child) && $child->can('rule_name') && defined($child->rule_name)) {
