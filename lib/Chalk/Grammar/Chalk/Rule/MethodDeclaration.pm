# ABOUTME: Semantic action for MethodDeclaration - creates FunctionDef with implicit $self
# ABOUTME: Methods are functions bound to a class context with $self as first parameter
use 5.42.0;
...
class Chalk::Grammar::Chalk::Rule::MethodDeclaration :isa(Chalk::GrammarRule) {
    method evaluate($context) {
        use Chalk::IR::Node::FunctionDef;

        # MethodDeclaration has two forms:
        # 1. 'method' WS_OPT Identifier WS_OPT '(' WS_OPT ParameterList WS_OPT ')' WS_OPT Block
        # 2. 'method' WS_OPT Identifier WS_OPT Block
        my @children = $context->children->@*;

        # Find method name - it's the first non-keyword, non-whitespace child
        # Child 0 is 'method', child 1 is WS_OPT, child 2 is Identifier
        my $method_name = $self->_extract_name($context->child(2));

        # Always start with $self as first parameter
        my @parameters = ('$self');
        my $body_node;
        ...
        if ($has_parens) {
            # Form 1: method name(params) block
            ...
            # Form 2: method name block
            # No additional parameters, body is last child
            $body_node = $context->child($#children);
        }
        ...
        # Get class context if available (for tracking method's class)
        my $env = $context->env;
        my $class_name = $env ? $env->{current_class} : undef;
    ...
    # Extract method name from Identifier result
