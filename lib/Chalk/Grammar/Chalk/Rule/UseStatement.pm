...
sub _extract_imports($context, $start_index) {
    ...
    for my $i ($start_index..$#children_array) {
        ...
        elsif (blessed($child) && $child->can('type') && $child->type eq 'quoted_word_list') {
...
class Chalk::Grammar::Chalk::Rule::UseStatement :isa(Chalk::GrammarRule) {
    method evaluate($context) {
        ...
        for my $i (0..$#children) {
            ...
            elsif (ref($child) eq 'HASH' && $child->{type} eq 'qualified_identifier') {
        ...
        return $builder->build_use_statement_node($type, $module_name, $imports);
    }
}
