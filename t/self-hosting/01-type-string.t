#!/usr/bin/env perl
# ABOUTME: Self-hosting test - compile Type::String to XS
...
# Test Type::String compilation
subtest 'Compile Chalk::IR::Type::String to XS' => sub {
    my $result = compile_module(
        'lib/Chalk/IR/Type/String.pm',
    ...
    TODO: {
        ...
        if ($result->{so_file} && -f $result->{so_file}) {
            ...
            my $loaded = eval { require Chalk::IR::Type::String; 1 };
            ok($loaded, 'Type::String module loaded from XS');

            # Test basic functionality
            if ($loaded) {
                my $type = eval { Chalk::IR::Type::String->new(); };
                ok(defined $type, 'Type::String object created');
                ok($type->is_top, 'Default Type::String is TOP') if defined $type;
            }
        }
    }
};
