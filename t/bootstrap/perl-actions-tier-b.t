...

{
    ...
    SKIP: {
        ...
        is(class_name_str($cls), 'Chalk::IR::Node::Constant', 'Constant.pm class name');
        is(class_parent_str($cls), 'Chalk::IR::Node', 'Constant.pm parent class');
        ...
        if (defined $meth) {
            # Implicit-return method `{ 'Constant' }` after Phase 3d:
            # body holds the bare value node (Constant), not a Return.
            # The Return is synthesized in the graph but isn't body[0].
            my $meth_body = method_body($meth);
            my $val = $meth_body->[0];
            ok(defined $val, 'Constant.pm return value: defined');
            is_constant($val, 'Constant', 'Constant.pm return value');
        }
    }
}
...

{
    ...
    SKIP: {
        ...
        # Return value should be Interpolate (legacy InterpolatedString)
        my $interp = $ret->inputs()->[1];
        ok(defined $interp && $interp->operation eq 'Interpolate',
            'Statement.pm interpolated string is Interpolate node');
...

{
    ...
    SKIP: {
        ...
        # Method with Interpolate (legacy InterpolatedString) containing 2 variables
        my $meth = find_method_in_body($body, 'emit');
        ...
        my $interp = $ret->inputs()->[1];
        ok(defined $interp && $interp->operation eq 'Interpolate',
            'Module.pm interpolated string is Interpolate node');
        my $parts = $interp->inputs()->[0];
