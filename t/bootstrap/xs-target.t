...
# === PMC Stub Generation (Prompt 9) ===

# Test: _generate_pmc returns PMC stub with default module name
{
    my $target = Chalk::Bootstrap::Target::XS->new();
    my $pmc = $target->_generate_pmc();

    like($pmc, qr/^# Generated by Chalk::Bootstrap compiler/, 'PMC has generated comment');
    like($pmc, qr/package Chalk::Grammar::BNF::Rules;/, 'PMC has default package name');
    like($pmc, qr/use v5\.42;/, 'PMC uses v5.42');
...
# Test: _generate_pmc uses configurable module name
{
...
# Test: _generate_build_pl returns Module::Build script
{
    ...
    like($build, qr/needs_compiler\s+=>\s+1/, 'Build.PL has needs_compiler');
