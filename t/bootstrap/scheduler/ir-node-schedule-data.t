...
my $sd   = Chalk::Scheduler::Roundtrip::Loop->new(node => $loop);
$loop->set_schedule_data($sd);
is($loop->schedule_data, $sd, 'set_schedule_data stores the meta');
isa_ok($loop->schedule_data, 'Chalk::Scheduler::Roundtrip::Loop');
...
my $sd_a = Chalk::Scheduler::Roundtrip::Loop->new(node => $bare_a, is_for_style => true);
