...
# so $node->class() still returns the legacy name expected by the analyzer.
sub ctor($class, %inputs) {
    ...
    if ($class eq 'MethodCallExpr') {
        return $typed->make('Call',
            ...
            name          => $inputs{method_name}->value(),
            inputs        => [$inputs{invocant}, $inputs{method_name}, $inputs{args}],
