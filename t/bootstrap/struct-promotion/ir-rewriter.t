...
# nodes (Add, Assign, NumEq, ...) — mirrors Chalk::IR::Shim::%BINOP_MAP.
my %BINOP_MAP = (
...
# so $node->class() still returns the legacy name expected by the rewriter.
sub ctor($class, %inputs) {
    ...
    if ($class eq 'BuiltinCall') {
        return $typed->make('Call',
            ...
            inputs        => [$inputs{name}, $inputs{args}],
