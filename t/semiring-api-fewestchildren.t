#!/usr/bin/env perl
# Test FewestChildren semiring API standardization for EvalContext support
use 5.42.0;
use experimental qw(class builtin);
use Test::More;
use Scalar::Util qw(refaddr);
use Chalk::Semiring::FewestChildren;
use Chalk::EvalContext;

# Test 1: init_element_from_rule accepts optional $ctx parameter
{
    my $semiring = Chalk::Semiring::FewestChildren->new();
...
{
    my $elem = Chalk::Semiring::FewestChildrenElement->new(
        valid       => 1,
        child_count => 0,
        context     => undef
    );

    ok($elem, "Can create FewestChildrenElement with context parameter");
