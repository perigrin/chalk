...
| `$1`..`$9` | `RegexCapture(match_node, n)` | a slot of the match node's result (the runtime-free-boundary doc's "value on a graph edge"): the captured bytes are COPIED into a fresh NUL-terminated buffer at the recorded offsets, upholding the backend invariant that every Str SSA value is NUL-terminated (length tracking is lost at phi merges and the strlen/`%s` fallbacks assume NUL — a zero-copy view printed the subject tail across a merge; views can return when the Str repr carries explicit lengths end-to-end). Reading an out-of-range group or a non-match input dies loudly. The undef face of a failed-match `$N` is a tracked follow-up (the guarded `matched ? $1 : ...` idiom — lib/'s dominant shape — never reads it). |
