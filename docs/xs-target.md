...
The PMC overlay mechanism provides a transparent upgrade path: the pure-Perl `.pm` file (produced by `Chalk::Target::Perl`) becomes a fallback, and the `.pmc` stub (produced by `Chalk::Target::XS`) loads the compiled XS version when available. Call sites are untouched — `use Chalk::Grammar::BNF::Rules` loads whichever implementation is present, preferring the compiled version.
...
| `lib/Chalk/Grammar/BNF/Rules.pmc` | XSLoader stub — transparent upgrade from pure Perl |
| `lib/Chalk/Grammar/BNF/Rules.pm` | Pure-Perl fallback (produced by `Chalk::Target::Perl`, unchanged) |
...
### 3.6 PMC Overlay Mechanism

Perl's module loader checks for `.pmc` files before `.pm` files. The PMC overlay exploits this to provide transparent XS acceleration:

```
lib/Chalk/Grammar/BNF/Rules.pm   ← pure-Perl fallback (Chalk::Target::Perl output)
lib/Chalk/Grammar/BNF/Rules.pmc  ← XSLoader stub (Chalk::Target::XS output)
...
2. Perl finds `.pmc` first → loads the XSLoader stub
3. XSLoader loads the compiled shared library (`.so` / `.dll`)
4. Rule methods are now XS functions in the symbol table
5. If XS is not compiled, remove or rename the `.pmc` → Perl loads the `.pm` fallback

**PMC stub content:**

```perl
# Generated by Chalk compiler — do not edit
...
Verify the transparent loading mechanism:

1. With `.pmc` present and XS compiled: module loads via XSLoader, methods are XS
2. With `.pmc` removed: module loads the `.pm` fallback, methods are Perl
3. Both configurations produce identical recognizer results
4. No call-site changes between configurations

### 5.5 Determinism

Regenerating the XS output from the same BNF input must produce byte-identical `.xs`, `.pmc`, and `Build.PL` files. This is validated by generating twice and diffing.
...
| PMC | `.pmc` file extension; Perl checks for `.pmc` before `.pm` during module loading. Used here as an XSLoader stub. |
